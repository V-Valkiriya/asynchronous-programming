 ...
 */
  ...
  const result = users.map((user) => `${user.id}. ${user.email}, ${user.phone}, ${user.website}`);
  const notIdFound = ids.some(id => id > 10 || id < 1);

    if (notIdFound) { 
      return Error(`404: Not Found: there is no user with id ${id}`);
    } else {
      return result;
    }
};
